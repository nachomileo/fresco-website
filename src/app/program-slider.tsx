      ...
      <section className="program-preview" aria-live="polite" ref={previewRef}>
        ...
        <div className="program-preview-grid">
          {activeGroup.items.map((item) => {
            const cardHref = activeGroup.slug === "talleres" || item.href === "/seminarios/autoedicion" ? item.href : undefined;
            return <article className="program-preview-card" key={item.number}>
              ...
              <div className="program-preview-body">
                ...
                <div><p>{item.artist}</p><h4>{cardHref ? <Link className="program-preview-card-link" href={cardHref}>{item.title}</Link> : item.title}</h4>{item.subtitle && <p className="program-preview-subtitle">{item.subtitle}</p>}<p className="program-preview-description">{item.description}</p></div>
                <div className="program-preview-footer"><strong>{item.detail}</strong><Link href={activeGroup.slug === "talleres" ? item.href ?? "/talleres" : activeGroup.slug === "musica" ? "mailto:info@fresco.art" : item.href ?? `/${activeGroup.slug}`}>{activeGroup.slug === "talleres" || activeGroup.slug === "musica" ? "Consultar" : activeGroup.slug === "seminarios" ? "Me apunto" : "Ver ficha"} <span aria-hidden="true">↗</span></Link></div>
              </div>
            </article>;
          })}
        </div>
